...
import { CellMeasurerCache, defaultCellRangeRenderer } from 'react-virtualized';
...
import { FixedColContainer, GridComponent, FixedGrid } from './gridStyles';
...
const GridWrapper = forwardRef((props, componentRef) => {
  const { current: cache } = useLazyRef(
    () =>
      new CellMeasurerCache({
        ...
        keyMapper: props.cacheKeyMapper,
      }),
  );
  ...
  const cellRangeRenderer = args =>
    props.cellRangeRenderer(args, defaultCellRangeRenderer);
  ...
  return (
    ...
    >
      {props.fixedColumnCount > 0 ? (
        <FixedColContainer className="sticky-col-container">
          <FixedGrid
...
GridWrapper.propTypes = {
  ...
  cacheKeyMapper: PropTypes.func,
  cellRangeRenderer: PropTypes.func,
};

GridWrapper.defaultProps = {
  type: 'body',
  cellRenderer: defaultCellRenderer,
  cellRangeRenderer: defaultRangeRenderer,
};
